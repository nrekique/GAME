...
@export var crosshair_blocked_color: Color = Color(1.0, 0.35, 0.35, 0.95)
...
func _handle_interact_pressed() -> void:
	...
	if target_node == null:
		return
	if not _can_interact_with(target_node):
		return
	if target_node.has_method("interact"):
		var result: Variant = target_node.call("interact", self)
		if result is bool and not bool(result):
			return
		if _is_physics_ball(target_node) and bool(target_node.call("is_held_by", self)):
			_held_ball = target_node
		return
...
func _update_interact_crosshair() -> void:
	...
	if _held_ball != null and is_instance_valid(_held_ball) and _held_ball.is_held_by(self):
		...
		if _can_interact_with(_current_interaction_target):
			_crosshair_dot.color = crosshair_target_color
		else:
			_crosshair_dot.color = crosshair_blocked_color
		_set_crosshair_dot_size(crosshair_dot_size + 1.0)
	else:
		_crosshair_dot.color = crosshair_idle_color
		_set_crosshair_dot_size(crosshair_dot_size)
...
func _can_interact_with(target_node: Node) -> bool:
	if target_node == null or not is_instance_valid(target_node):
		return false
	if target_node.has_method("can_interact"):
		var can_v: Variant = target_node.call("can_interact", self)
		if can_v is bool:
			return bool(can_v)
	return true
...
func _is_physics_ball(node: Node) -> bool:
	return node != null and node.get_script() == PHYSICS_BALL_SCRIPT
